...
Puppet::Type.type(:iis_application).provide(:webadministration, parent: Puppet::Provider::IIS_PowerShell) do
  ...
  def create
    check_paths
    # fail('must provide either :sitename or :virtualdirectory') if @resource[:sitename].nil? && @resource[:virtualdirectory].nil?
  ...
  def self.compare_app_names(app, resource)
    ...
    resource_appname =  resource[:applicationname].split(/[\\\/]/) - Array(find_sitename(resource))
